...
function fmt(v: number | null | undefined): { text: string; neg: boolean; zero: boolean } {
...
// Suma todos los keys del objeto (incluyendo 01-PRINCIPAL)
...
interface TabProps { plan: PlanQuery; filtros: FiltroDashboard; }

function CcPills({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
    <div className="mb-4 flex flex-wrap gap-2">
      {CENTROS.map((cc) => (
        <button key={cc.key} onClick={() => onChange(cc.key)}
          className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${value === cc.key ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}>
          {cc.label}
        </button>
      ))}
    </div>
...
// ─────────────────────────────────────────────
// TAB PERÍODO
// Usa v_eri_resumida via useEri (filtrada por CC si aplica)
// ─────────────────────────────────────────────
function TabPeriodo({ plan, filtros }: TabProps) {
  ...
  const { planRows, valueMap, ingresosTotales } = useMemo(() => {
    ...
    // orden 15 = fila total ingresos en plan_pyg (usada para % verticales)
  ...
  const periodoLabel =
    filtros.año === "Todas" ? "Acumulado total"
    : filtros.mes === "Todos" ? `Acumulado ${filtros.año}`
    : mesLabel((filtros.año as number) * 100 + (filtros.mes as number));

  const rowBg = (nivel: string, val: number) => {
    if (nivel === "Titulo") return { bg: "#1e2d42", bold: true };
    if (nivel === "Total") return { bg: val >= 0 ? "#1a2d1a" : "#2d1a1a", bold: true };
    if (nivel === "Subtotal") return { bg: "#0d2040", bold: true };
    return { bg: "transparent", bold: false };
  };

  return (
    <div>
      <CcPills value={ccActivo} onChange={setCcActivo} />
      <p className="mb-3 text-xs text-muted-foreground">{periodoLabel} · {ccLabel}</p>
      {isLoading ? (
        <div className="space-y-2">{Array.from({ length: 12 }).map((_, i) => <LoadingSkeleton key={i} className="h-7 w-full" />)}</div>
      ) : isError ? <ErrorState /> : planRows.length === 0 ? <EmptyState /> : (
        <div className="overflow-auto rounded-lg border border-border">
          <table className="w-full border-collapse text-[11px]">
            <thead className="sticky top-0 z-10 bg-card">
              <tr className="border-b border-border">
                <th className="w-[55%] px-3 py-2 text-left text-xs font-semibold uppercase tracking-wider text-muted-foreground">Concepto</th>
                <th className="px-3 py-2 text-right text-xs font-semibold uppercase tracking-wider text-muted-foreground">Valor</th>
                <th className="px-3 py-2 text-right text-xs font-semibold uppercase tracking-wider text-muted-foreground">% Ingr.</th>
              </tr>
            </thead>
            <tbody>
              {planRows.map((row) => {
                ...
                const f = fmt(val);
                const { bg, bold } = rowBg(row.nivel, val);
                const isTitulo = row.nivel === "Titulo";
                return (
                  <tr key={row.orden} className="border-b border-border/40" style={{ background: bg, fontWeight: bold ? 700 : 400 }}>
                    <td className="px-3 py-1.5 text-foreground">{row.etiqueta_fila || row.concepto}</td>
                    {isTitulo ? <><td /><td /></> : (
                      <>
                        <td className={`whitespace-nowrap px-3 py-1.5 text-right tabular-nums ${f.neg ? "text-destructive" : f.zero ? "text-muted-foreground" : "text-foreground"}`}>{f.text}</td>
                        <td className="px-3 py-1.5 text-right tabular-nums text-muted-foreground">{pct != null ? `${pct.toFixed(2)}%` : ""}</td>
                      </>
                    )}
                  </tr>
                );
...
// ─────────────────────────────────────────────
// TAB MES A MES
// Usa v_eri_resumida (sin tercero) para todos los totales
// ─────────────────────────────────────────────
function TabMesAMes({ plan, filtros }: TabProps) {
  ...
  // Construir mapa: orden → mes → valor
  ...
  const sub = useMemo(() => {
    const iCuentas = planRows.filter((r) => r.nivel === "Cuenta" && String((r as any).clase_cod) === "4" && String((r as any).grupo_cod) === "41");
    const cCuentas = planRows.filter((r) => r.nivel === "Cuenta" && String((r as any).clase_cod) === "6");
    const oiCuentas = planRows.filter((r) => r.nivel === "Cuenta" && String((r as any).clase_cod) === "4" && String((r as any).grupo_cod) === "42");
    ...
    return {
      ingMes, ing: sum(ingMes),
      costMes, cost: sum(costMes),
      ubMes, ub: sum(ubMes),
      gopMes, gop: sum(gopMes),
      uoMes, uo: sum(uoMes),
      oiMes, oi: sum(oiMes),
      gnopMes, gnop: sum(gnopMes),
      uaiMes, uai: sum(uaiMes),
    };
  ...
  const MC = ({ v, bold = false }: { v: number; bold?: boolean }) => {
    const f = fmt(v);
    return (
      <td className={`whitespace-nowrap px-3 py-1.5 text-right tabular-nums text-[11px] ${bold ? "font-bold" : ""} ${f.neg ? "text-destructive" : f.zero ? "text-muted-foreground/30" : "text-foreground"}`}>
        {f.text}
      </td>
    );
  };
  ...
  const CuentaRow = ({ row }: { row: PlanPygRow }) => {
    ...
    return (
      <tr className="border-b border-border/40 bg-background/20">
        ...
        {mVals.map((v, i) => <MC key={i} v={v} />)}
        <td className={`whitespace-nowrap px-3 py-1.5 text-right font-semibold tabular-nums text-[11px] ${f.neg ? "text-destructive" : f.zero ? "text-muted-foreground" : "text-foreground"}`}>{f.text}</td>
        <td className="whitespace-nowrap px-3 py-1.5 text-right text-[10px] tabular-nums text-muted-foreground">{pct != null && tot !== 0 ? `${pct.toFixed(2)}%` : "-"}</td>
      </tr>
    );
  };
  ...
  const TotalRow = ({ label, mv, tot }: { label: string; mv: number[]; tot: number }) => {
    ...
    return (
      <tr className="border-b border-border font-bold" style={{ background: tot >= 0 ? "#1a2d1a" : "#2d1a1a" }}>
        ...
        {mv.map((v, i) => <MC key={i} v={v} bold />)}
        <td className={`whitespace-nowrap px-3 py-1.5 text-right font-bold tabular-nums text-[11px] ${tot < 0 ? "text-destructive" : tot === 0 ? "text-muted-foreground" : "text-foreground"}`}>{f.text}</td>
        <td className="whitespace-nowrap px-3 py-1.5 text-right text-[10px] tabular-nums text-muted-foreground">{pct != null && tot !== 0 ? `${pct.toFixed(2)}%` : "-"}</td>
      </tr>
    );
  };
  ...
  const SubRow = ({ label, mv, tot }: { label: string; mv: number[]; tot: number }) => {
    ...
    return (
      <tr className="border-b border-border border-l-2 border-l-primary font-bold" style={{ background: "#0d2040" }}>
        ...
        {mv.map((v, i) => <MC key={i} v={v} bold />)}
        <td className={`whitespace-nowrap px-3 py-1.5 text-right font-bold tabular-nums text-[11px] ${tot < 0 ? "text-destructive" : tot === 0 ? "text-muted-foreground" : "text-primary"}`}>{f.text}</td>
        <td className="whitespace-nowrap px-3 py-1.5 text-right text-[10px] tabular-nums text-muted-foreground">{pct != null && tot !== 0 ? `${pct.toFixed(2)}%` : "-"}</td>
      </tr>
    );
  };
  ...
  const SH = ({ label }: { label: string }) => (
    <tr style={{ background: "#1e2d42" }}>
      <td colSpan={months.length + 3} className="px-3 py-2 text-[11px] font-bold uppercase tracking-wide text-foreground">{label}</td>
    </tr>
  );

  const ingCuentas = planRows.filter((r) => r.nivel === "Cuenta" && String((r as any).clase_cod) === "4" && String((r as any).grupo_cod) === "41");
  ...
  return (
    <div>
      ...
      {isLoading ? (
        <div className="space-y-2">{Array.from({ length: 12 }).map((_, i) => <LoadingSkeleton key={i} className="h-7 w-full" />)}</div>
      ) : isError ? <ErrorState /> : planRows.length === 0 || months.length === 0 ? <EmptyState /> : (
        <div className="overflow-auto rounded-lg border border-border">
          <table className="w-full border-collapse text-[11px]">
            <thead className="sticky top-0 z-10 bg-card">
              <tr className="border-b border-border">
                ...
                {months.map((m) => <th key={m} className="whitespace-nowrap px-3 py-2 text-right text-xs font-semibold uppercase tracking-wider text-muted-foreground">{mesLabel(m)}</th>)}
                ...
                <th className="min-w-[60px] whitespace-nowrap px-3 py-2 text-right text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">% Vert.</th>
              </tr>
            </thead>
            <tbody>
              <SH label="INGRESOS OPERACIONALES" />
              {ingCuentas.map((r) => <CuentaRow key={r.orden} row={r} />)}
              <TotalRow label="TOTAL INGRESOS OPERACIONALES" mv={sub.ingMes} tot={sub.ing} />
              <SH label="COSTOS DE VENTAS" />
              {costCuentas.map((r) => <CuentaRow key={r.orden} row={r} />)}
              ...
              {gastCuentas.map((r) => <CuentaRow key={r.orden} row={r} />)}
              ...
              {oiCuentas.map((r) => <CuentaRow key={r.orden} row={r} />)}
...
// Usa v_eri_resumida para todos los totales por CC
// ─────────────────────────────────────────────
interface VCC { [cc: string]: number; }
interface N1 { tipo_gasto: string; valores: VCC; detalles: N2[]; }
interface N2 { detalle_gasto: string; valores: VCC; cuentas: N3[]; }
interface N3 { nombre_cuenta: string; valores: VCC; terceros: N4[]; }
interface N4 { tercero: string; nit: string; valores: VCC; }
...
function buildTree(rows: GastoTerceroRow[]): N1[] {
  ...
  for (const [key, ccMap] of map.entries()) {
    ...
    if (!n2) { n2 = { detalle_gasto: det, valores: {}, cuentas: [] }; n1.detalles.push(n2); }
    for (const [cc, v] of Object.entries(vals)) n2.valores[cc] = (n2.valores[cc] ?? 0) + v;
    let n3 = n2.cuentas.find((c) => c.nombre_cuenta === nom);
    if (!n3) { n3 = { nombre_cuenta: nom, valores: {}, terceros: [] }; n2.cuentas.push(n3); }
  ...
  return Array.from(n1Map.values()).sort((a, b) => a.tipo_gasto.localeCompare(b.tipo_gasto))
    .map((n1) => ({ ...n1, detalles: n1.detalles.sort((a, b) => a.detalle_gasto.localeCompare(b.detalle_gasto))
      .map((n2) => ({ ...n2, cuentas: n2.cuentas.sort((a, b) => a.nombre_cuenta.localeCompare(b.nombre_cuenta))
...
function TabPorCC({ plan, filtros }: TabProps) {
  ...
  // Mapa orden → cc → valor (desde v_eri_resumida)
  ...
  const togSec = (k: string) => setOpenSec((p) => { const s = new Set(p); s.has(k) ? s.delete(k) : s.add(k); return s; });
  const tog = (set: React.Dispatch<React.SetStateAction<Set<string>>>) => (k: string) =>
    set((p) => { const s = new Set(p); s.has(k) ? s.delete(k) : s.add(k); return s; });
  ...
  const gop = tree.find((n) => n.tipo_gasto === "01.GASTOS OPERACIONALES")?.valores ?? {};
  const vUO: VCC = {};
  for (const cc of CC_KEYS) vUO[cc.key] = (vUB[cc.key] ?? 0) - (gop[cc.key] ?? 0);
  ...
  const gnop = tree.find((n) => n.tipo_gasto === "02.GASTO NO OPERACIONAL")?.valores ?? {};
  const vUAI: VCC = {};
  const allCC = new Set([...CC_KEYS.map((c) => c.key), ...Object.keys(vOI), ...Object.keys(gnop)]);
  for (const cc of allCC) vUAI[cc] = (vUO[cc] ?? 0) + (vOI[cc] ?? 0) - (gnop[cc] ?? 0);

  const año = filtros.año === "Todas" ? 2026 : Number(filtros.año);
  const meses = [{ value: "Todos" as const, label: "Acumulado" },
  ...
  const CV = (vals: VCC, cc: { key: string }, bold = false) => {
    const f = fmt(vals[cc.key] ?? 0);
    return <td key={cc.key} className={`${col} ${bold ? "font-bold" : ""} ${f.neg ? "text-destructive" : f.zero ? "text-muted-foreground/30" : "text-foreground"}`}>{f.text}</td>;
  };
  const CC_ = (vals: VCC, bold = false) => {
    const v = sumAll(vals); const f = fmt(v);
    return <td className={`${col} ${bold ? "font-bold" : ""} ${f.neg ? "text-destructive" : f.zero ? "text-muted-foreground/30" : "text-primary"}`}>{f.text}</td>;
  };
  const PCT = (vals: VCC, cc: { key: string }, bold = false) => {
    const ci = vIng[cc.key] ?? 0; const v = vals[cc.key] ?? 0;
    if (!v || !ci) return <td className="px-2 py-1.5 text-right text-[10px] text-muted-foreground/20 whitespace-nowrap min-w-[55px]">-</td>;
    return <td className={`px-2 py-1.5 text-right text-[10px] tabular-nums whitespace-nowrap min-w-[55px] ${bold ? "font-bold" : ""} text-muted-foreground/70`}>{((v/ci)*100).toFixed(2)}%</td>;
  };
  const PCTC = (vals: VCC, bold = false) => {
    const v = sumAll(vals);
    if (!v) return <td className="px-2 py-1.5 text-right text-[10px] whitespace-nowrap min-w-[60px] text-muted-foreground/20">-</td>;
    return <td className={`px-2 py-1.5 text-right text-[10px] tabular-nums whitespace-nowrap min-w-[60px] ${bold ? "font-bold" : ""} text-muted-foreground/70`}>{((v/ingTotal)*100).toFixed(2)}%</td>;
  };
  ...
  const SH = ({ label, k }: { label: string; k: string }) => (
    <tr style={{ background: "#1e2d42" }} className="cursor-pointer hover:opacity-90" onClick={() => togSec(k)}>
      <td colSpan={CC_KEYS.length * 2 + 3} className="px-3 py-2 text-[11px] font-bold uppercase tracking-wide text-foreground">
        <span className="mr-2 text-[10px] text-muted-foreground">{openSec.has(k) ? "▼" : "▶"}</span>{label}
      </td>
    </tr>
  );
  ...
  const TR = ({ label, vals }: { label: string; vals: VCC }) => (
    <tr className="border-b border-border" style={{ background: sumAll(vals) >= 0 ? "#1a2d1a" : "#2d1a1a" }}>
      <td className="px-3 py-1.5 text-[11px] font-bold text-foreground">{label}</td>
      {CC_KEYS.map((cc) => <Fragment key={cc.key}>{CV(vals,cc,true)}{PCT(vals,cc,true)}</Fragment>)}
      {CC_(vals,true)}{PCTC(vals,true)}
    </tr>
  );
  ...
  // Fila de cuenta (sin detalle de tercero — simplificado para consistencia)
  const CR = ({ row, idx }: { row: PlanPygRow; idx: number }) => {
    ...
    return (
      <tr className={`border-b border-border/20 ${idx % 2 === 0 ? "bg-background/10" : ""}`}>
        ...
        {CC_KEYS.map((cc) => <Fragment key={cc.key}>{CV(vals,cc)}{PCT(vals,cc)}</Fragment>)}
        {CC_(vals)}{PCTC(vals)}
      </tr>
    );
  };
  ...
  const GasTree = ({ n1 }: { n1: N1 }) => {
    const k1 = n1.tipo_gasto; const o1 = openN1.has(k1);
    return (
      <Fragment key={k1}>
        <tr className="cursor-pointer border-b border-border/40 hover:opacity-90" style={{ background: "#151f33" }} onClick={() => tog(setOpenN1)(k1)}>
          <td className="px-3 py-1.5 pl-5 text-[11px] font-semibold text-foreground">
            <span className="mr-2 text-[10px] text-muted-foreground">{o1 ? "▼" : "▶"}</span>{n1.tipo_gasto}
          </td>
          {CC_KEYS.map((cc) => <Fragment key={cc.key}>{CV(n1.valores,cc,true)}{PCT(n1.valores,cc,true)}</Fragment>)}
          {CC_(n1.valores,true)}{PCTC(n1.valores,true)}
        </tr>
        {o1 && n1.detalles.map((n2) => {
          const k2 = `${k1}||${n2.detalle_gasto}`; const o2 = openN2.has(k2);
          if (sumAll(n2.valores) === 0) return null;
          return (
            <Fragment key={k2}>
              <tr className="cursor-pointer border-b border-border/30 hover:opacity-90" style={{ background: "#0d1525" }} onClick={() => tog(setOpenN2)(k2)}>
                <td className="px-3 py-1.5 pl-10 text-[11px] text-muted-foreground">
                  <span className="mr-2 text-[10px] text-muted-foreground/60">{o2 ? "▾" : "▸"}</span>{n2.detalle_gasto}
                </td>
                {CC_KEYS.map((cc) => <Fragment key={cc.key}>{CV(n2.valores,cc)}{PCT(n2.valores,cc)}</Fragment>)}
                {CC_(n2.valores)}{PCTC(n2.valores)}
              </tr>
              {o2 && n2.cuentas.map((n3) => {
                const k3 = `${k2}||${n3.nombre_cuenta}`; const o3 = openN3.has(k3);
                if (sumAll(n3.valores) === 0) return null;
                return (
                  <Fragment key={k3}>
                    <tr className="cursor-pointer border-b border-border/10 hover:opacity-80" onClick={() => tog(setOpenN3)(k3)}>
                      <td className="px-3 py-1 pl-16 text-[11px] text-muted-foreground/80">
                        <span className="mr-2 text-[10px] text-muted-foreground/40">{o3 ? "▾" : "▸"}</span>{n3.nombre_cuenta}
                      </td>
                      {CC_KEYS.map((cc) => <Fragment key={cc.key}>{CV(n3.valores,cc)}{PCT(n3.valores,cc)}</Fragment>)}
                      {CC_(n3.valores)}{PCTC(n3.valores)}
                    </tr>
                    {o3 && n3.terceros.map((t, ti) => {
                      const ct = sumAll(t.valores); const ft = fmt(ct);
                      return (
                        <tr key={`${k3}||${ti}`} className="border-b border-border/10" style={{ background: "#080c18" }}>
                          <td className="px-3 py-0.5 pl-20">
                            <div className="flex items-center gap-2">
                              {t.nit && t.nit !== "SIN TERCERO" && <span className="flex-shrink-0 rounded bg-muted/40 px-1 font-mono text-[9px] text-muted-foreground">{t.nit}</span>}
                              {(() => { const a = CC_KEYS.filter((cc) => (t.valores[cc.key] ?? 0) !== 0); if (a.length === 1) return <span className="flex-shrink-0 rounded bg-primary/20 px-1 font-mono text-[9px] text-primary/70">{a[0].label}</span>; return null; })()}
                              <span className="text-[10px] text-muted-foreground/60">{t.tercero}</span>
                            </div>
                          </td>
                          {CC_KEYS.map((cc) => { const f = fmt(t.valores[cc.key] ?? 0); return (<Fragment key={cc.key}><td className={`${col} text-[10px] ${f.neg ? "text-destructive" : f.zero ? "text-muted-foreground/20" : "text-muted-foreground/50"}`}>{f.text}</td><td className="px-2 py-0.5 min-w-[55px]" /></Fragment>); })}
                          <td className={`${col} text-[10px] ${ft.neg ? "text-destructive" : "text-muted-foreground/50"}`}>{ft.text}</td>
    ...
    <div>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {meses.map((m) => (
            <button key={String(m.value)} onClick={() => setMesLocal(m.value as any)}
        ...
        <div className="flex gap-2">
          <button onClick={() => { setOpenN1(new Set(tree.map((n)=>n.tipo_gasto))); setOpenN2(new Set(tree.flatMap((n)=>n.detalles.map((d)=>`${n.tipo_gasto}||${d.detalle_gasto}`)))); setOpenN3(new Set(tree.flatMap((n)=>n.detalles.flatMap((d)=>d.cuentas.map((c)=>`${n.tipo_gasto}||${d.detalle_gasto}||${c.nombre_cuenta}`)))))); }}
            className="rounded-md border border-border px-3 py-1 text-xs text-muted-foreground transition-colors hover:bg-muted hover:text-foreground">Expandir todo</button>
          <button onClick={() => { setOpenN1(new Set()); setOpenN2(new Set()); setOpenN3(new Set()); }}
            className="rounded-md border border-border px-3 py-1 text-xs text-muted-foreground transition-colors hover:bg-muted hover:text-foreground">Contraer todo</button>
        </div>
      </div>

      {isLoading ? (
        <div className="space-y-1">{Array.from({length:8}).map((_,i)=><div key={i} className="h-8 animate-pulse rounded bg-muted/40"/>)}</div>
      ) : (
        <div className="overflow-auto rounded-lg border border-border">
          <table className="w-full border-collapse text-[11px]">
            <thead className="sticky top-0 z-10 bg-card">
              <tr className="border-b border-border">
                ...
                {CC_KEYS.map((cc) => (
                  <Fragment key={cc.key}>
                    <th className="min-w-[110px] whitespace-nowrap px-2 py-2 text-right text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">{cc.label}</th>
